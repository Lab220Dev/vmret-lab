...
const props = defineProps({
    ...
    products: {
        //products é um array de objetos
...
const formatDateTime = (date) => {
    //formatDateTime é uma função que recebe uma data como parâmetro
    return date ? format(new Date(date), 'dd/MM/yyyy HH:mm') : null; //retorna a data formatada ou null se não houver data
};

const truncatedText = (text) => {
    //truncatedText é uma função que recebe um texto como parâmetro
...
<template>
    ...
    <DataTable :value="props.products" removableSort :rows="5" size="Normal" columnResizeMode="fit" responsiveLayout="scroll" class="mt-3">
        <template #header>
            <div class="flex flex-wrap items-center justify-between gap-2">
                <span class="text-xl mb-3">{{ $t('latest_withdrawals') }}</span>
...
.tooltip-target {
    /* Estilo para o elemento que exibirá o tooltip */
