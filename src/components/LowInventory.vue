    ...
    <DataTable :value="props.low" removableSort :rows="5" size="normal" columnResizeMode="fit" responsiveLayout="scroll" class="mt-3">
        <template #header>
            <div class="flex flex-wrap items-center justify-between gap-2">
                <span class="text-xl mb-3">{{ $t('low_inventory_items') }}</span>
            </div>
        </template>
        <Column field="sku" style="width: 10%" :header="t('SKU')" sortable>
            <!-- Coluna de SKU -->
            <template #body="{ data }">
                <!-- Template para exibir o SKU com tooltip -->
                <span class="tooltip-target" v-tooltip="data.sku">{{ data.sku }}</span>
                <!-- Exibe o SKU com tooltip -->
            </template>
        </Column>
        ...
        <Column field="nome" :header="t('item')" sortable>
            <!-- Coluna de Produto -->
            <template #body="{ data }">
                <!-- Template para exibir o nome do produto com tooltip -->
                <span class="tooltip-target" v-tooltip.top="data.nome">{{ data.nome }}</span>
        ...
        <template #empty>
            ...
            <!-- Exibe a mensagem de vazio definida em 'emptyMessage' -->
        </template>
    </DataTable>
</template>
...
.tooltip-target {
    /* Define o estilo do elemento com tooltip */
