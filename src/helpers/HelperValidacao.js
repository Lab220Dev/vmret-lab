...
export const isCentroCustoExists = async (centroCusto) => {
    ...
    console.log('Plantas no dataStore:', dataStore.cdcs);
    ...
    console.log('setor carregados:', cdcs);
    ...
    if (isNumeric(centroCusto)) {
        ...
        console.log('Verificando nome do cdc:', centroCusto);
...
export const isSetorExists = async (setor) => {
    const dataStore = useDataStore(); // Acessa o estado global de dados, obtendo uma instância do dataStore.
    console.log('Setor no dataStore:', dataStore.setores);
    ...
    console.log('setor carregados:', setores);

    // Função para normalizar e remover acentuação
    ...
    if (isNumeric(setor)) {
        ...
        console.log('Verificando nome do setor:', setor);
        // Caso contrário, verifica se o nome do Setor (label) existe nos Setores
        return setores.some(({ label }) => normalizeString(label) === normalizeString(setor)); // Remove acentuação e compara
    }
};
...
export const isPlantaExists = async (planta) => {
    const dataStore = useDataStore(); // Acessa o estado global de dados, obtendo uma instância do dataStore.
    console.log('Plantas no dataStore:', dataStore.plantas);
    ...
    if (isNumeric(planta)) {
        ...
        console.log('Verificando ID da Planta:', planta);
        ...
        console.log('Verificando nome da Planta:', planta);
...
export const isFuncaoExists = async (funcao) => {
    ...
    if (isNumeric(funcao)) {
        ...
        console.log('Verificando nome da funcao:', funcao);
        return funcoes.some(({ nome }) => funcao.toLowerCase() === nome.toLowerCase()); // Retorna `true` se o nome da Função for igual ao fornecido (ignora maiúsculas/minúsculas), caso contrário `false`.
    }
};
...
export const validarCNPJ = (cnpj) => {
    ...
    let soma = 0,
        pos = tamanho - 7; // Inicializa as variáveis para os cálculos.
