...
const ListaItens = ref([]); // Declara um array reativo chamado ListaItens
const ListaClientes = ref([]); // Declara um array reativo chamado ListaClientes
const ListaProdutos = ref([]); // Declara um array reativo chamado ListaProdutos
...
const ListaDMS = ref([]); // Declara um array reativo chamado ListaDMS
const controladoraRefs = ref([]); // Declara um array reativo chamado controladoraRefs
...
const loadData = async () => { // Declara uma função assíncrona chamada loadData
    loading.value = true; // Ativa o estado de loading
    try { // Inicia um bloco try para capturar possíveis erros
        const produtos = dataStore.produtos || (await dataStore.fetchProdutos()); // Busca os produtos do dataStore ou faz uma requisição para buscá-los
        ListaProdutos.value = produtos
            ...
            .map(({ value, codigo, label }) => ({
                label: `${codigo} | ${label}`, // Formata o label do produto
                value: value
            }))
            .sort((a, b) => {
        ...
        toast.add({ severity: 'error', summary: t('title_error'), detail: t('load_initial_data'), life: 3000 }); // Adiciona uma mensagem de erro ao toast
        console.error('Erro ao carregar dados iniciais:', error); // Loga a mensagem de erro no console
    } finally {
        loading.value = false; // Desativa o estado de loading
    }
};
