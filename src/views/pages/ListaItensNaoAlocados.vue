...
import { ref, onMounted } from 'vue';  // Importa o Vue para uso de referências reativas e lifecycle hooks
...
const emptyMessage = ref('Nenhum item encontrado');  // Mensagem padrão quando não há itens retornados
...
const fetchItensNaoAlocadas = async () => {
    ...
    try {
        ...
        if (response.data.length === 0) {
            emptyMessage.value = 'Nenhum produto registrado';  // Atualiza a mensagem de "nenhum item encontrado"
        }
        ...
        if (error.response && error.response.status === 404) {
            emptyMessage.value = 'Nenhum produto registrado';  // Atualiza a mensagem de erro se não encontrar dados
        } else {
            // Caso contrário, exibe uma mensagem de erro genérica
            emptyMessage.value = 'Erro ao carregar itens não alocados';  // Exibe uma mensagem padrão de erro
...
<template>
    <div class="card vh">  <!-- Cartão de conteúdo -->
        <h5 class="my-4 text-2xl">Itens não alocados</h5>  <!-- Título da seção -->
            ...
            <template #header>
                <div class="flex justify-content-between align-items-center">
                    <div class="flex justify-content-start">
                        <span>Total de registros: {{ itens.length }}</span>  <!-- Exibe o número total de registros -->
                    </div>
                    <div>
                        <IconField iconPosition="left">
                            ...
                            <InputText v-model="filters['global'].value" placeholder="Busca" />  <!-- Campo de entrada para busca global -->
                        </IconField>
                    </div>
                </div>
            </template>
